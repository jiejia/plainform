...
export default async function Submission({ params }: SubmissionProps) {
    ...
    return (
        <DashboardLayout breadcrumbs={<><Link href={"/dashboard"}>Dashboard</Link> / <Link href={"/dashboard/form"}>Form</Link> / <Link href={"/dashboard/form/" + id}>{res.data.title}</Link> / <span>Submissions</span></>} menuItemId={2}>
            <Index form={res.data} />
        </DashboardLayout>
    );
}